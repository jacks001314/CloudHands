...
static int _dns_rr_parse(ch_pool_t *mp,ch_dns_data_input_t *din,struct list_head *list,uint16_t n){
	...
	for(i = 0;i<n;i++){
		...
		rdata = ch_dns_rdata_parse(mp,din);
		if(rdata == NULL){
...
ch_dns_response_t* ch_dns_response_parse(ch_pool_t *mp,ch_dns_data_input_t *din){

	uint16_t i;
    ch_dns_header_t tmp_header,*header = &tmp_header;

	ch_dns_question_t *dnsq = NULL;
	ch_dns_response_t *dnsr;

	if(ch_dns_header_parse(din,header)){
    ...
    dnsr = (ch_dns_response_t*)ch_pcalloc(mp,sizeof(*dnsr));

    ch_dns_header_clone(header,&dnsr->hdr);
	...
	if(dnsr->hdr.ancount){
		...
		if(_dns_rr_parse(mp,din,&dnsr->alist,dnsr->hdr.ancount)){
	...
	if(dnsr->hdr.aucount){
		...
		if(_dns_rr_parse(mp,din,&dnsr->aulist,dnsr->hdr.aucount)){
	...
	if(dnsr->hdr.adcount){
		...
		if(_dns_rr_parse(mp,din,&dnsr->adlist,dnsr->hdr.adcount)){
