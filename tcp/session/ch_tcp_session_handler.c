...
ch_tcp_session_handler_create(ch_pool_t *mp,ch_tcp_context_t *tcp_context,uint32_t task_id){
	...
	shandler = (ch_tcp_session_handler_t*)ch_palloc(mp,sizeof(*shandler));

	shandler->tcp_context = tcp_context;
	shandler->task_id = task_id;
	shandler->mp = mp;

    snprintf(ptable_name,64,"TCPSessionHandler_%lu",(unsigned long)task_id);
	...
	shandler->pstore = ch_proto_session_store_create(mp,task_id,
    ...
    if(tcp_context->use_mpa){
        ...
        shandler->mpa = ch_mpool_agent_create(mp,
