...
ch_tcp_session_request_handler_create(ch_pool_t *mp,
	...
	req_handler = (ch_tcp_session_request_handler_t*)ch_palloc(mp,sizeof(*req_handler));

	req_handler->shandler = shandler;
	req_handler->mp = mp;
    req_handler->ta_pool = ta_pool;
    req_handler->tcp_context = tcp_context;

    snprintf(ptable_name,64,"TCPSessionRequestHandler_%lu",(unsigned long)shandler->task_id);
	req_handler->req_pool = ch_tcp_session_request_pool_create(tcp_context,0,NULL,NULL,ptable_name);
...
_tcp_session_data_process(ch_tcp_session_request_handler_t *sreq_handler,ch_tcp_session_t *tcp_session,ch_packet_tcp_t *tcp_pkt){
	...
	return ch_tcp_session_packet_handle(sreq_handler->shandler,tcp_session,tcp_pkt);
}
...
_tcp_session_create(ch_tcp_session_request_handler_t *req_handler,
	...
	void *sentry = ch_tcp_app_session_entry_create(req_handler->shandler->mpa,app,req_handler->shandler->pstore);
	if(sentry == NULL)
	{	
...
int ch_tcp_session_request_packet_handle(ch_tcp_session_request_handler_t *req_handler,ch_packet_tcp_t *tcp_pkt){
	...
	do{
		app = ch_tcp_app_find_by_port(req_handler->ta_pool,req_handler->shandler->pstore,tcp_pkt); 
